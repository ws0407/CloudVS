...
def dl_remote(local_path):
    ...
    for filename in re.findall("nvdcve-1.1-.*\.json\.zip", r.text):
...
def save_cve_to_redis(cve_dict):
    ...
    r = redis.StrictRedis(host='192.168.220.1', port=6379, db=0)
...
def save_app_cve_map_to_redis(cve_dict):
    ...
    r = redis.StrictRedis(host='192.168.220.1', port=6379, db=1)
    cve_feed = cve_dict['CVE_Items']
    for cve in cve_feed:
        cve_id = cve['cve']["CVE_data_meta"]["ID"]
...
if __name__ == '__main__':
    # dl_remote('/kvm/db/cve_db/')
    # print("finished downloading cve database")
    cve_dict = load_local('/kvm/db/cve_db')
